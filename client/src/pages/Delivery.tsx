...
export default function Delivery() {
  ...
  const [calculatingFee, setCalculatingFee] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (session && sessionTime) {
      ...
      // Session expires after 12 hours (shorter for delivery personnel)
    ...
    if (isAuthenticated) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', {
          ...
          schema: 'public',
          table: 'orders',
          filter: `store_name=eq.Grandpa's Supermart`
        }, () => {
          fetchOrders();
        })
        .subscribe();
          ...
          {!showForgotPassword ? (
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Enter your delivery code"
              ...
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-800 disabled:cursor-not-allowed text-white py-4 rounded-xl font-black uppercase text-sm tracking-widest transition-all active:scale-95 shadow-lg shadow-blue-500/20 disabled:shadow-none"
              >
                {isLoggingIn ? (
              ...
              <button
                ...
                className="w-full text-zinc-500 hover:text-blue-500 text-xs font-bold uppercase tracking-wider transition-colors"
              >
                Forgot Password?
              </button>
            </form>
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full bg-zinc-900 hover:bg-zinc-800 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest transition-all"
              >
                Back to Login
              </button>
            </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button
                onClick={fetchOrders}
                disabled={loading}
                className="bg-zinc-900 p-2.5 rounded-xl hover:bg-zinc-800 transition-all active:scale-95 disabled:opacity-50"
              >
                <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
              </button>
              <button
                onClick={handleLogout}
                className="bg-zinc-900 p-2.5 rounded-xl hover:bg-red-600 transition-all active:scale-95"
              >
                <LogOut size={18} />
              </button>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            ...
            <div className="bg-zinc-950 border border-zinc-900 px-4 py-2 rounded-xl flex items-center gap-2 whitespace-nowrap">
              ...
              <span className="text-xs font-black">
                {orders.filter(o => o.status === 'SHIPPED').length} En Route
              </span>
            </div>
          </div>
        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
                  <div className="bg-zinc-950 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl p-6">
                    ...
                    <div className="space-y-4">
                      {/* Distance Input */}
                      <div>
                        <label className="text-xs font-black text-zinc-500 uppercase mb-2 block">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // Auto-calculate fee: ₦200 base + ₦150 per km
                            const distance = parseFloat(e.target.value) || 0;
                            const fee = 200 + (distance * 150);
                            setFeeInput(fee.toFixed(2));
                          }}
                      ...
                      {feeInput && (
                        <div className="bg-blue-500/10 border border-blue-500/30 p-4 rounded-xl text-center">
                          ...
                          <p className="text-xs text-zinc-500 mt-2">
                            Base: ₦200 + ({distanceInput}km × ₦150/km)
                          </p>
                        </div>
                      )}
                      ...
                      <div>
                        <label className="text-xs font-black text-zinc-500 uppercase mb-2 block">
                          Delivery Fee (Override if needed)
                        </label>
                        <input
                          type="number"
                          step="0.01"
                      ...
                      <button
                        onClick={async () => {
                          if (!distanceInput || !feeInput) {
                            alert('Please enter distance and fee');
                            return;
                          }
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${
                      ...
                      order.status === 'SHIPPED' ? 'bg-green-600/20 text-green-500' :
                      order.status === 'PREPARING' ? 'bg-yellow-600/20 text-yellow-500' :
              ...
              <div className="bg-black/50 border border-zinc-900 p-4 rounded-2xl space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    {order.phone_number && (
                      <p className="text-xs text-green-400 font-bold mt-1">📞 {order.phone_number}</p>
                    )}
                  </div>
                </div>
