              ...
              import { Heart, ShoppingBag, Search, Edit3, Save, Trash2, CheckCircle2, Send, RefreshCw } from 'lucide-react';
              ...
              export default function Home() {
                ...
                // --- 5. DATA FETCHING (Defined before useEffect to avoid hoisting errors) ---
                ...
                const handleConfirmOrder = async () => {
                  if (!clientName || !address || cart.length === 0) return;
                  ...
                  return (
                    <div className="min-h-screen bg-black flex items-center justify-center p-6 text-white animate-in zoom-in">
                      <div className="bg-[#111] border border-orange-500/20 p-12 rounded-[3.5rem] text-center max-w-sm w-full shadow-2xl">
                        ...
                        <p className="text-orange-500 text-[9px] font-black uppercase mb-10 tracking-[0.4em]">{storeName}</p>
                ...
                return (
                  <div className={`min-h-screen bg-black text-white font-sans transition-all duration-700 ${isUiHidden ? 'grayscale brightness-50' : ''}`}>
                    ...
                    {/* CATEGORIES - FIXED JSX ERRORS FROM IMAGE 947462 */}
                    ...
                    {showCheckout && (
                      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
                        <div className="bg-[#0c0c0c] border border-zinc-900 w-full max-w-lg rounded-[4rem] p-12 shadow-2xl animate-in zoom-in">
                          <h2 className="text-4xl font-black italic uppercase text-center mb-10">Your Bag</h2>
                          <div className="space-y-3 mb-10 max-h-[150px] overflow-y-auto no-scrollbar pr-2">
                            {cart.map((item, i) => (
                              <div key={i} className="bg-black border border-zinc-900 p-5 rounded-2xl flex justify-between items-center group">
                                ...
                                <button onClick={() => { const n = [...cart]; n.splice(i,1); setCart(n); }} className="text-red-900 text-[8px] font-black uppercase opacity-0 group-hover:opacity-100 transition-opacity">Remove Item</button>
                              </div>
                            ))}
                          ...
                          <div className="space-y-4 mb-10">
                            ...
                            <input placeholder="SHIPPING ADDRESS" value={address} onChange={e => setAddress(e.target.value.toUpperCase())} className="w-full bg-black border border-zinc-900 rounded-2xl py-6 px-8 text-[11px] font-black uppercase outline-none focus:border-orange-500" />
                          </div>
                          <div className="flex justify-between items-center mb-10 px-4 border-t border-zinc-900 pt-8">
                            ...
                            <span className="text-2xl font-black italic text-orange-500">${cart.reduce((s, i) => s + i.price, 0).toFixed(2)}</span>
                          </div>
                          <button onClick={handleConfirmOrder} className="w-full bg-orange-600 text-white py-7 rounded-[2.5rem] font-black uppercase text-xs tracking-[0.2em] shadow-2xl active:scale-95 transition-all">Confirm Order</button>
                          <button onClick={() => setShowCheckout(false)} className="w-full mt-6 text-[9px] font-black uppercase text-zinc-600 tracking-widest hover:text-white transition-colors text-center">Close Bag</button>
                        </div>
                      </div>
                    )}
